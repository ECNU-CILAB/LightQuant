...
def plot_figure(avg_train_loss_list=[], avg_val_loss_list=[], avg_acc_list=[], avg_mcc_list=[], epochs=100):
    ...
    # 绘制验证损失曲线
    plt.subplot(1, 3, 3)
    plt.plot(range(1, epochs // 10 + 1), avg_val_loss_list, label='Average Validation Loss', color='blue')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    ...
    # 绘制准确率曲线
    plt.subplot(1, 3, 2)
    plt.plot(range(1, epochs // 10 + 1), avg_acc_list, label='Average Accuracy', color='orange')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    ...
    # 绘制MCC曲线
    plt.subplot(1, 3, 3)
    plt.plot(range(1, epochs // 10 + 1), avg_mcc_list, label='Average MCC', color='green')
    plt.xlabel('Epochs')
    plt.ylabel('MCC')
