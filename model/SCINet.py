...
class SCINet_Layer(nn.Module):
    def __init__(self, output_len, input_len, input_dim = 9, hid_size = 1, num_stacks = 1,
        ...
        super(SCINet_Layer, self).__init__()
...
class SCINet(nn.Module):
    def __init__(self, input_len, pred_len, input_dim, hidden_dim, SCINet_Layers):
        super().__init__()
        self.scinet = SCINet_Layer(
            output_len=pred_len - 1,
            input_len=input_len,
            input_dim=input_dim,
            hid_size=hidden_dim,
            num_levels=SCINet_Layers,
            concat_len=0,
            dropout=0.3
        )
    ...
    def forward(self, x):
        ...
        out = self.scinet(x)  # 输出: [batch, pred_len, features]
        out = self.output_layer(out)  # 转换为概率 [batch, pred_len, 1]
        return out.squeeze(-1)  # 输出: [batch, pred_len]
